...
import {
  GRID_W, GRID_H, RESPAWN_TICKS, ROUND_DURATION_TICKS, KILL_SCORE,
} from '@battle-city/shared'
import type { GameState, Tank, TileType, InputEvent } from '@battle-city/shared'
...
function baseState(overrides: Partial<GameState> = {}): GameState {
  return {
    ...
    map: emptyMap(),
    spawnPoints: [{ x: 32, y: 32 }, { x: 96, y: 96 }],
    tanks: {},
    bullets: {},
    players: {},
    hostId: 'host',
...
describe('tickGame', () => {
  ...
  it('transitions to ended when roundEndsAt tick is reached', () => {
    ...
    const next = tickGame(state, [])
    expect(next.roundPhase).toBe('ended')
  })
  ...
  it('does not tick game logic when phase is ended', () => {
    ...
    const next = tickGame(state, [input])
  ...
  it('spawns a bullet when shoot input is true', () => {
    const tank = aliveTank('p1', 64, 64)
    const state = baseState({ tanks: { p1: tank } })
    const input: InputEvent = { playerId: 'p1', tick: 0, moveDir: null, shoot: true }
    const next = tickGame(state, [input])
  ...
  it('does not spawn a second bullet if player already has one in flight', () => {
    const tank = aliveTank('p1', 64, 64)
    const bullet = { id: 'b1', ownerId: 'p1', x: 80, y: 64, direction: 'right' as const }
    const state = baseState({ tanks: { p1: tank }, bullets: { b1: bullet } })
    const input: InputEvent = { playerId: 'p1', tick: 0, moveDir: null, shoot: true }
    const next = tickGame(state, [input])
    expect(Object.keys(next.bullets).length).toBe(1)
  })
  ...
  it('removes a bullet that hits a brick wall and destroys the tile', () => {
    const tank = aliveTank('p1', 16, 16)
    // Place brick at tile (3, 1) = pixel (48, 16)
    const map = emptyMap()
    map[1][3] = 'brick'
    const bullet = { id: 'b1', ownerId: 'p1', x: 44, y: 16, direction: 'right' as const }
    const state = baseState({ tanks: { p1: tank }, bullets: { b1: bullet }, map })
    const next = tickGame(state, [])
  ...
  it('kills a tank hit by an enemy bullet and credits the shooter', () => {
    const shooter = aliveTank('p1', 16, 64)
    const target = aliveTank('p2', 64, 64)
    const bullet = { id: 'b1', ownerId: 'p1', x: 60, y: 64, direction: 'right' as const }
    const state = baseState({ tanks: { p1: shooter, p2: target }, bullets: { b1: bullet } })
    const next = tickGame(state, [])
  ...
  it('starts respawn countdown on a killed tank', () => {
    ...
    const target = aliveTank('p2', 64, 64)
    const bullet = { id: 'b1', ownerId: 'p1', x: 60, y: 64, direction: 'right' as const }
    const state = baseState({ tick: 50, tanks: { p1: shooter, p2: target }, bullets: { b1: bullet } })
    const next = tickGame(state, [])
    expect(next.tanks['p2'].respawnTick).toBe(50 + RESPAWN_TICKS)
  })

  it('respawns a dead tank when respawnTick is reached', () => {
    const tank = { ...aliveTank('p2', 64, 64), alive: false, respawnTick: 10 }
    const state = baseState({ tick: 10, tanks: { p2: tank } })
    const next = tickGame(state, [])
    expect(next.tanks['p2'].alive).toBe(true)
  })
})
